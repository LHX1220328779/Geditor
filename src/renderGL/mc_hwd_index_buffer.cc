...
void HwdIndexBuffer::Create(const IndexBuffer *ibuffer) {
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, ibuffer->GetNumBytes(), 0,
               static_cast<unsigned int>(ibuffer->GetUsage()));
...
void HwdIndexBuffer::Disable() { glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0); }

void *HwdIndexBuffer::Lock(Buffer::LockMode mode) {
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, handle_);
  void *videoMemory =
      glMapBuffer(GL_ELEMENT_ARRAY_BUFFER, static_cast<unsigned int>(mode));
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
  return videoMemory;
}
...
bool HwdIndexBuffer::IsValidate() const { return (handle_ != 0); }

unsigned int HwdIndexBuffer::GetHandle() const { return handle_; }
