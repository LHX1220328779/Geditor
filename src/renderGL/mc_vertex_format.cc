...
VertexFormat::VertexFormat() : num_attributes_(0), stride_(0) {
...
int VertexFormat::GetIndex(AttributeUsage usage,
                           unsigned int usageIndex) const {
  for (int i = 0; i < num_attributes_; ++i) {
    if (elements_[i].usage == usage && elements_[i].usage_index == usageIndex) {
      return i;
    }
  }
...
int VertexFormat::GetStride() const { return stride_; }

int VertexFormat::GetNumAttributes() const { return num_attributes_; }
...
VertexFormat::AttributeType VertexFormat::GetAttributeType(
    int attribute) const {
  if (0 <= attribute && attribute < num_attributes_) {
...
VertexFormat::AttributeUsage VertexFormat::GetAttributeUsage(
    int attribute) const {
  if (0 <= attribute && attribute < num_attributes_) {
...
void VertexFormat::AppendAttribute(AttributeUsage usage, AttributeType type,
...
int VertexFormat::GetTypeSize(AttributeType type) { return type_size_[type]; }
