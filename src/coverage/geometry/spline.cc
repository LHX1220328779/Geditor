...
void Spline::GetPath(SiteVec& list, SiteVec& anchors, double density) {
  list.clear();
  X.clear();
  Y.clear();
  for (auto& p : anchors) {
  ...
  gener.set_points(X, Y);

  double x = anchors.front().x;
  double end_x = anchors.back().x;

  for (; x < end_x; x += density) {
    double y = gener(x);
