#include "coverage/geometry/reeds_shepp.h"
...
ReedsSheppStateSpace::sample_many(
    double q0[3], double q1[3], double step_size,
    std::vector<std::vector<PointInfo>> &data,
    std::vector<std::vector<Polygon>> &bounding_polygon) {
  float front_lon = 1.2;
  float back_lon = 0.4;
  float half_lat = 0.6;
  ReedsSheppPath path = reedsShepp(q0, q1);
  ...
  while (!pathqueue.empty()) {
    ...
    std::vector<PointInfo> single;
    PointInfo pt;
    std::vector<Polygon> polygons;

    // vector< vector<double> > temp_data;
    vector<double> mid_data;
    for (double seg = 0.0; seg <= dist; seg += step_size) {
      interpolate(q0, path, seg / rho_, qnew);
      temp.x = qnew[0];
      temp.y = qnew[1];
...
ReedsSheppStateSpace::sample_many(
    ...
    std::vector<std::vector<PointInfo>> &data,
    std::vector<std::vector<Polygon>> &bounding_polygon,
    std::vector<std::vector<std::vector<double>>> &temp_data) {
  // float front_lon = 1.2;
  // float back_lon = 0.4;
  // float half_lat = 0.6;
  ...
  while (!pathqueue.empty()) {
    ...
    // std::vector< std::vector<double> > mid_data1, reverse_data;
    std::vector<std::vector<double>> mid_data1;
    for (double seg = 0.0; seg <= dist; seg += step_size) {
      std::vector<double> mid_data;
      interpolate_1(q0, path, seg / rho_, qnew);
      // cout << "zhangzhersrsrs" << qnew[0] << "::" << qnew[1] <<  "::" <<
      // qnew[2] <<  "::" << qnew[3] << endl;
      ...
      mid_data.push_back(qnew[0]);
      mid_data.push_back(qnew[1]);
      mid_data.push_back(qnew[2]);
      mid_data.push_back(qnew[3]);
      mid_data1.push_back(mid_data);
      //     static double current_direction = qnew[3];
      //     ROS_INFO_STREAM("1current_direction is: " << current_direction);
      //     if(current_direction == qnew[3])
      //     {
      //         ROS_WARN_STREAM("Normal Process. ");
      //         if(qnew[3] == 0)
      //         {
      //             mid_data1.push_back(mid_data);
      //             ROS_ERROR_STREAM("Push_back forward point. " <<
      //             mid_data1.size());
      //         }

      //         else
      //         {
      //             reverse_data.push_back(mid_data);
      //             ROS_ERROR_STREAM("Push_back backward point. " <<
      //             reverse_data.size());
      //         }
      //     }
      //     else
      //     {
      //         ROS_WARN_STREAM("Reversing pathpoints. ");
      //         current_direction = qnew[3];
      //         ROS_INFO_STREAM("2current_direction is: " <<
      //         current_direction); if(!reverse_data.empty())
      //         {
      //             ROS_ERROR_STREAM("BEFORE: reverse.front() : " <<
      //             reverse_data.front().front() << "reverse.back() : " <<
      //             reverse_data.back().back()); reverse(reverse_data.begin(),
      //             reverse_data.end()); ROS_ERROR_STREAM("AFTER:
      //             reverse.front() : " << reverse_data.front().front()<<
      //             "reverse.back() : " << reverse_data.back().back());
      //         }
      //     if(!mid_data1.empty())
      //     {
      //         ROS_ERROR_STREAM("BEFORE INSERTING: " << mid_data1.size() << "
      //         "<< mid_data1.back().back()) ;
      //         mid_data1.insert(mid_data1.end(), reverse_data.begin(),
      //         reverse_data.end()); ROS_ERROR_STREAM("AFTER INSERTING: " <<
      //         mid_data1.size() << " "<< mid_data1.back().back()) ;
      //         reverse_data.clear();
      //     }
      //     else
      //     {
      //         ROS_ERROR_STREAM("BEFORE SWAPING: " << mid_data1.size() << "
      //         "<< reverse_data.size()) ; mid_data1.swap(reverse_data);
      //         ROS_ERROR_STREAM("AFTER SWAPING: " << mid_data1.size() << " "<<
      //         reverse_data.size()) ;
      //     }
      // }
    ...
    // bounding_polygon.push_back(polygons);
    // zhangzhe
    temp_data.push_back(mid_data1);
  }

  return patharray;
}

void ReedsSheppStateSpace::interpolate_1(double q0[3], ReedsSheppPath path,
                                         double seg, double s[4])
...
// double seg, double s[3])
{
  ...
  for (unsigned int i = 0; i < 5 && seg > 0; ++i) {
    // cout << "path::" << path.length << "::" << path.length[i] << endl;
    if (path.length_[i] < 0) {
    ...
    phi = s[2];
    // cout << "path::2" << path.type_  << endl;
    // switch (path.type_[i])
    int value = *(path.type_ + i);
    switch (value) {
      case RS_LEFT:
    ...
    if (0 == i) {
      ...
      if (hypot(compare[0] - s[0], compare[1] - s[1]) <
  ...
  if (path.length_[counts] < 0) {
    flag = 1;
  } else if (path.length_[counts] > 0) {
    flag = 0;
  } else {
  }
...
void ReedsSheppStateSpace::GetPath(SiteVec &list, Site start, Site end,
                                   double density) {
  double RS_START_POINT[3] = {start.x, start.y, start.angle};
  double RS_END_POINT[3] = {end.x, end.y, end.angle};
  vector<ReedsSheppStateSpace::ReedsSheppPath> patharry;
  vector<vector<PointInfo>> data;
  vector<vector<Polygon>> bounding_polygon;
  vector<vector<vector<double>>> temp_data;
  vector<vector<vector<double>>> mid_temp_data;

  patharry = sample_many(RS_START_POINT, RS_END_POINT, density, data,
                         bounding_polygon, temp_data);
  if (!temp_data.empty()) {
    for (auto p : temp_data[0]) {
      list.emplace_back(p[0], p[1]);
    }
  }
}
}  // namespace geometry
