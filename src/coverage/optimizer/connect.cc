...
void Connect::GetPath(SiteVec &list, SiteVec &list1, SiteVec &list2) {
  ...
  int num_of_connect = connect_path.size();
  double angle_of_connect_head =
      (connect_path[1] - connect_path[0]).inerangle();
  double angle_of_connect_tail =
      (connect_path[num_of_connect - 1] - connect_path[num_of_connect - 2])
          .inerangle();
  if (std::fabs(angle_of_connect_head - angle_of_connect_tail) > 100) {
    geometry::Dubins dbs;
    Site dubins_start(connect_path.front());
    Site dubins_end(connect_path.back());
    dubins_start.angle = angle_of_connect_head;
    dubins_end.angle = angle_of_connect_tail;
    SiteVec dubins_connect;
    dbs.GetPath(dubins_connect, dubins_start, dubins_end, 1.1);
    connect_path.swap(dubins_connect);
  }
