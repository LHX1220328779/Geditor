...
int PurePursuit::Smooth(SiteVec& path) {
  ...
  double delta = 0;
  while (last_near_ < raw_path_.size() - 1) {
    // std::cout << "[last_near]" << last_near_ << "," << raw_path_.size()<<
    // std::endl;
    Control(delta);
    Update(delta);
  ...
  for (auto it = update_path_.begin(); it != update_path_.end() - 1; ++it)
    it->angle = (*(it + 1) - *it).inerangle();
...
int PurePursuit::TargetInd() {
  ...
  std::ofstream out_file("/Users/fengyounan/Desktop/error", std::ios::app);
  if (last_near_ < 10) {
    for (size_t i = 1; i < last_near_ + 10 && i < raw_path_.size(); ++i) {
    ...
    out_file << min << std::endl;
  } else if (last_near_ >= 10) {
    for (size_t i = last_near_ - 10;
         i < last_near_ + 10 && i < raw_path_.size(); ++i) {
      double tmp_dis = (status_ - raw_path_.at(i)).mold();
      if (tmp_dis < min) {
        min = tmp_dis;
        nearest_ind = i;
  ...
  out_file.close();
  if (last_near_ > nearest_ind) {
    last_near_++;
    target_index_ = last_near_;
  } else {
  ...
  while (Lf_ > length && target_index_ + 1 < raw_path_.size()) {
    length +=
        (raw_path_.at(target_index_ + 1) - raw_path_.at(target_index_)).mold();
    ++target_index_;
    // std::cout << "[total lenghth]" << length  << std::endl;
  }
  return 0;
}
...
int PurePursuit::Control(double& delta) {
  ...
  double alpha =
      std::atan2(target.y - status_.y, target.x - status_.x) - status_.angle;
