...
void Math::GetArc(SiteVec &list, Site &center, double &radius, Site &base,
                  double length) {
  list.clear();
  ...
  if (-1 == radius) {
    Site target(base + base.direction());
    list.push_back(base);
    list.push_back(target);
    optimizer::MeanFilt mft;
    mft.Interpolation(list);
  }
  ...
  for (size_t i = 1; i < count; ++i) {
    ...
    if (clockwise) {
      new_site =
          Site(center.x + radius * std::cos(base_angle - i * delta_alpha),
    ...
    // std::cout << "new_site = " << new_site.x << "," << new_site.y <<
    // std::endl;
  }

  for (size_t i = 0; i < list.size() - 1; ++i)
    list[i].angle = (list[i + 1] - list[i]).inerangle();
