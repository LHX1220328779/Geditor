...
class Matrix4x4 {
 ...
 public:
  ...
  Matrix4x4(N f11, N f12, N f13, N f14, N f21, N f22, N f23, N f24, N f31,
            N f32, N f33, N f34, N f41, N f42, N f43, N f44);
 ...
 public:
  ...
  static Matrix4x4<N> LookAt(const VectorType &eye, const VectorType &center,
                             const VectorType &up);
  ...
  static Matrix4x4<N> Frustum(N left, N right, N bottom, N top, N zNear,
                              N zFar);
...
template <typename N>
Matrix4x4<N>::Matrix4x4(N f11, N f12, N f13, N f14, N f21, N f22, N f23, N f24,
                        N f31, N f32, N f33, N f34, N f41, N f42, N f43,
                        N f44) {
  m11 = f11;
  m12 = f12;
  m13 = f13;
...
Matrix4x4<N> Matrix4x4<N>::Multiply(const Matrix4x4<N> &mat) {
  ...
  for (int j = 0; j < 4; j++) {
    for (int i = 0; i < 4; i++) {
      m[j][i] = mat1.m[j][0] * mat.m[0][i] + mat1.m[j][1] * mat.m[1][i] +
                mat1.m[j][2] * mat.m[2][i] + mat1.m[j][3] * mat.m[3][i];
    }
  }
...
Matrix4x4<N> Matrix4x4<N>::Inverse() const {
  ...
  return Matrix4x4<N>(d00, d01, d02, d03, d10, d11, d12, d13, d20, d21, d22,
                      d23, d30, d31, d32, d33);
}
...
Vector3<N> Matrix4x4<N>::TransformNormal(const VectorType &vec) const {
  return VectorType(m11 * vec[0] + m21 * vec[1] + m31 * vec[2],
                    m12 * vec[0] + m22 * vec[1] + m32 * vec[2],
...
template <typename N>
Matrix4x4<N> Matrix4x4<N>::Ortho(N left, N right, N bottom, N top, N zNear,
                                 N zFar) {
  Matrix4x4<N> prjMat;
...
template <typename N>
Matrix4x4<N> Matrix4x4<N>::Frustum(N left, N right, N bottom, N top, N zNear,
                                   N zFar) {
  Matrix4x4<N> prjMarix;
...
template <typename N>
Matrix4x4<N> Matrix4x4<N>::LookAt(const VectorType &eye,
...
Matrix4x4<N> operator*(const Matrix4x4<N> &mat1, const Matrix4x4<N> &mat2) {
  ...
  for (int j = 0; j < 4; j++) {
    for (int i = 0; i < 4; i++) {
      mat.m[j][i] = mat1.m[j][0] * mat2.m[0][i] + mat1.m[j][1] * mat2.m[1][i] +
                    mat1.m[j][2] * mat2.m[2][i] + mat1.m[j][3] * mat2.m[3][i];
    }
  }
