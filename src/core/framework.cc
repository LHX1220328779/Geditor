...
void Framework::Set2DView() {
  ...
  if (m_pCamera == &m_StereoCamera) m_fMapScale *= 2.414;
...
void Framework::Set3DView() {
  ...
  if (m_pCamera == &m_PlanCamera) m_fMapScale /= 2.414;
...
void Framework::HighlightSegment(double x, double y, int type) {
  ...
  if (m_pHighFeature != reslut.pFeatureObject) {
    ...
    if (m_pHighFeature) {
      if (type == 0) m_pHighFeature->SetHighlightState(true);
...
int Framework::SetSegmentRelation() {
  ...
  if (laneSegments.empty()) return -1;
  if (attchRoadAreas.empty() && attchSignBoards.empty()) return 0;
  for (int i = 0; i < laneSegments.size(); i++) {
...
int Framework::SetTrafficStopline() {
  ...
  if (arrSignboard.empty()) return -1;
  if (arrStopline.empty()) return 0;
...
void Framework::ActiveLayer(LayerType layertype) {
  if (m_pCamera != &m_PlanCamera) Set2DView();
...
int Framework::CreateReverseLaneGroup() {
  ...
  for (auto &b : bl) b = CreateGeoLine(geoline_type_);
...
int Framework::CreateReverseLaneGroup2() {
  ...
  float wl = w_lane_ + wm;
  if (ori->CopyGeometry(wm, bl[0]) < 0) return 0;
  if (ori->CopyGeometry(wl, bl[1]) < 0) return 0;
  if (ori->CopyGeometry(-wm, bl[2]) < 0) return 0;
  if (ori->CopyGeometry(-wl, bl[3]) < 0) return 0;
  bl[0]->ReverseVertex();
  bl[1]->ReverseVertex();
  std::vector<BoundSegment *> bds;
  for (auto &b : bl) {
...
int Framework::CreateBoundaryByLane() {
  ...
  for (auto &b : bl) b = CreateGeoLine(geoline_type_);
  float wl = w_lane_;
  if (ori->CopyGeometry(wl, bl[0]) < 0) return 0;
  if (ori->CopyGeometry(-wl, bl[1]) < 0) return 0;
  std::vector<BoundSegment *> bds;
  for (auto &b : bl) {
    auto *bd = new BoundSegment();
...
void Framework::FillPointsOnSelectedBoundary(double interval) {
    ...
    // 清除选择
    for (auto* f : m_selectedSet) f->SetSelectedState(false);
    m_selectedSet.clear();
...
static double CoverageScore(const std::vector<Point3d> &lanePts,
                            ...
                            double distTol) {
  if (lanePts.empty() || trajPts.empty()) return 0.0;

  // 中心线采样点覆盖率：落在轨迹走廊内的比例
  int laneHit = 0;
  for (const auto &p : lanePts)
    if (PointToPolylineMinDist(p, trajPts) <= distTol) ++laneHit;
  ...
  for (const auto &p : trajPts)
    if (PointToPolylineMinDist(p, lanePts) <= distTol) ++trajHit;
  double trajCov = (double)trajHit / trajPts.size();

  // 调和均值：两端都要覆盖才能得高分
  if (laneCov + trajCov < 1e-6) return 0.0;
...
int Framework::AutoBindTrajectory(double distTol, double coverageThreshold,
  ...
  if (lanes.empty()) {
    ...
    if (diag) diag->detail = detail.str();
    return 0;
  }
  ...
  auto collectPts = [](const std::vector<Point3d> &pts, int maxN)
      -> std::vector<Point3d> {
    if ((int)pts.size() <= maxN) return pts;
  ...
  for (auto *seg : m_trajectorylayer->GetAllTrajectory())
    if (addBoundSeg(seg, "TrajectoryLayer")) ++trajLayerCnt;
  int boundLayerCnt = 0;
  for (auto *seg : m_boundlayer->GetAllBoundary())
    if (addBoundSeg(seg, "BoundaryLayer")) ++boundLayerCnt;
  int segLayerCnt = 0;
  for (auto *lane : lanes) {
    auto *prop = lane->GetProperty();
    if (prop->mineSegmentIndex <= 0) continue;
    TrajInfo ti;
    lane->GetGeometry()->Hermite(ti.pts);
    if (ti.pts.empty()) continue;
    ti.sampledPts = collectPts(ti.pts, 100);
    ti.mineIndex = prop->mineSegmentIndex;
    std::strncpy(ti.mineCode, prop->mineSegmentCode, sizeof(ti.mineCode) - 1);
    ti.mineCode[sizeof(ti.mineCode) - 1] = '\0';
  ...
  int unboundCnt = 0;
  for (auto *lane : lanes)
    if (lane->GetProperty()->mineSegmentIndex <= 0) ++unboundCnt;
...
bool Framework::PickTrajectoryAnchor(double x, double y, double tolerance,
  ...
  if (!m_trajectorylayer) return false;
...
LaneSegment *Framework::GenerateCenterlineFromAnchors(BoundSegment *traj,
  ...
  if (idxStart == idxEnd) return nullptr;
...
int Framework::GenerateCenterlineChainByDirection(
  ...
  if (!m_trajectorylayer) return 0;

  const auto &trajs = m_trajectorylayer->GetAllTrajectory();
  if (trajs.empty()) return 0;
  ...
  for (auto *s : trajs) adj[s];  // 确保每个节点存在
  ...
  if (!found) {
    if (diag) diag->detail = "无法在轨迹图中找到连通路径";
    return 0;
  }

  std::vector<BoundSegment *> chain;
  for (BoundSegment *p = segB; p; p = prev[p]) chain.push_back(p);
  std::reverse(chain.begin(), chain.end());
  if (diag) diag->chainSegments = (int)chain.size();
  ...
  for (size_t i = 0; i < chain.size(); ++i) {
    ...
    if (i == 0) startIdx = idxA;
    if (i == chain.size() - 1) endIdx = idxB;
    if (i > 0) startIdx = nearestEndIdx(s, chain[i - 1]);
    if (i + 1 < chain.size()) endIdx = nearestEndIdx(s, chain[i + 1]);
    if (startIdx == endIdx) {
  ...
  for (auto *ln : generated) ln->SetChanged(true);
...
int Framework::AutoCheckRoadRight(double threshold,
  ...
  for (auto *lane : lanes) {
    ...
    for (int i = 0; i < nv; ++i) {
      ...
      if (dl < worstLeft) worstLeft = dl;
      if (dr < worstRight) worstRight = dr;
    }
    ...
    detail << " minLeft=" << std::fixed << std::setprecision(2) << worstLeft
           ...
           << (compliant ? "1" : "0") << "\n";
    if (diag) {
      if (compliant) ++diag->compliant;
      else ++diag->nonCompliant;
    }
  }
  if (diag) diag->detail = detail.str();
  return updated;
...
int Framework::CheckLaneConnectivity(ConnectivityDiagnostic *diag) {
  ...
  if (isolated > 0) {
    detail << "⚠ 孤立车道ID: ";
    for (int id : isolatedIds) detail << id << " ";
    detail << "\n";
  }
  if (diag) {
    diag->totalLanes = (int)lanes.size();
...
bool Framework::ExportRoadRightTxt(const std::string &path,
  ...
  for (auto *lane : lanes) {
    ...
    if (p->mineSegmentCode[0] == '\0') continue;
...
double RayPolylineHit(const Point3d &P, double dx, double dy,
  ...
  for (int j = 0; j + 1 < m; ++j) {
    ...
    if (std::fabs(D) < 1e-12) continue;  // 平行
    double rx = A.x - P.x;
    double ry = A.y - P.y;
...
// 接缝局部加密：保留输入所有顶点；只对距离两端 seamZone 米以内的每条边
// 插入一个中点（密度 ×2），其余长边保持原密度，避免边界折线爆炸。
...
void DensifySeamLocal(const std::vector<Point3d> &in,
  ...
  auto inSeam = [&](double s) {
    if (hasStartSeam && s <= seamZone) return true;
    if (hasEndSeam && (total - s) <= seamZone) return true;
    return false;
  };
  out.push_back(in[0]);
  for (int i = 0; i + 1 < n; ++i) {
...
ProjResult ProjectOntoPolyline(const Point3d &p,
  ...
  if (bestJ == 0 && bestT <= 1e-9) r.atEndpoint = true;
  else if (bestJ == m - 2 && bestT >= 1.0 - 1e-9) r.atEndpoint = true;
  return r;
}
...
int Framework::GenerateAdaptiveBoundaries(
  ...
  if (!m_Segmentlayer) return 0;
  auto lanes = m_Segmentlayer->GetAllLane();
  if (diag) diag->totalLanes = (int)lanes.size();

  std::ostringstream os;
  os << std::fixed << std::setprecision(2);
  os << "参数: 最低车道宽=" << params.min_lane_width
     << "m, 建议车道宽=" << params.recommended_lane_width
     << "m, 边界间距=" << params.boundary_gap
     << "m, taper长度=" << params.taper_len << "m\n";

  // =========================================================
  // 几何原理说明（重要）
  // =========================================================
  // 设对向两条中心线 A(上山)、B(下山)。定义 d_A[i] = 顶点 A[i] 到 B 的最近点距离。
  // 模式判定（对整条对偶，任一顶点失效即降级）：
  //   全对偶双开：  min_i d_A[i] >= 2 * min_lane_width  且  min_j d_B[j] >= 2 * min_lane_width
  //   否则为单开
  // 宽度分配：
  //   双开：w_A[i] = d_A[i]/2 - gap；w_B[j] = d_B[j]/2 - gap
  //   单开：下山(B) 两侧均为 recommended_lane_width
  //         上山(A) 两侧均为 d_A[i] - recommended_lane_width - gap
  // 核心几何不变量（避免对向边界相交）：每条车道“靠对向一侧”的边界必须位于
  //   中线 (self 和 opposite 的等距中垂面) 之本侧，否则强制裁剪。
  // =========================================================
  ...
  auto findOpposite = [&](LaneSegment *self) -> LaneSegment * {
    ...
    int wantDir = 0;
    if (prop->direction == 1) wantDir = 2;
    else if (prop->direction == 2) wantDir = 1;
    ParallelSegment *pp = self->GetParallelSegment();
    if (pp) {
      for (int revId : {pp->leftReverseSegment, pp->rightReverseSegment}) {
        if (revId == 0) continue;
        for (auto *cand : lanes) {
          if (cand != self && cand->GetUniqueID() == revId) return cand;
        }
      }
    }
    if (wantDir == 0) return nullptr;
    auto *sg = self->GetGeometry();
    if (!sg || sg->GetVertexCount() == 0) return nullptr;
    ...
    for (auto *cand : lanes) {
      ...
      if (!cp || cp->direction != wantDir) continue;
  ...
  if (m_trajectorylayer) allTrajs = m_trajectorylayer->GetAllTrajectory();
  auto trajForLane = [&](LaneSegment *lane) -> geditor::BoundSegment * {
    ...
    for (auto *tr : allTrajs) {
      ...
      if (!tp) continue;
      if (tp->mineSegmentIndex == idx && idx > 0) return tr;
    }
    return nullptr;
  };
  ...
  for (auto *lane : lanes) {
    ...
    if (!g || g->GetVertexCount() < 2) continue;
    std::vector<Point3d> raw;
    int n = g->GetVertexCount();
    raw.reserve(n);
    for (int i = 0; i < n; ++i) raw.push_back(g->GetVertex(i));
  ...
  for (auto *lane : lanes) {
    ...
    double threshold = 2.0 * params.min_lane_width;
    double minA = std::numeric_limits<double>::max();
    double minB = std::numeric_limits<double>::max();
    // 只看纵向重叠区内的 d
    for (int i = 0; i < (int)ld.d.size(); ++i)
      if (ld.inOverlap[i] && ld.d[i] < minA) minA = ld.d[i];
    for (int i = 0; i < (int)od.d.size(); ++i)
      if (od.inOverlap[i] && od.d[i] < minB) minB = od.d[i];
  ...
  // ---- 第二遍：填 halfInner/halfOuter ----
  double rec = params.recommended_lane_width;
  ...
  auto fillHalves = [&](LaneData &ld) {
    ...
    auto halfAt = [&](int i, double &hIn, double &hOut) {
      ...
      if (ld.dualWide) {
        ...
        // 未标方向：按建议宽度但受中线约束 (d-gap)/2
        double cap = d * 0.5 - gap * 0.5;
        if (cap < 0.2) cap = 0.2;
        double h = std::min(rec, cap);
        hIn = h;
        hOut = h;
      }
    };
    ...
    // 第二轮：超出 oppo 纵向范围的段，用最近的 inside 顶点的 halfInner/halfOuter
    // 外推（而不是随 d 爆炸）。这就避免了"超出对向时边界向外扩张"。
    if (firstIn < 0) {
      // 根本没有重叠：全段按建议宽度填
      for (int i = 0; i < n; ++i) {
        ld.halfInner[i] = rec;
  ...
  for (auto *lane : lanes) {
    ...
    if (it->second.hasOpposite) {
      if (it->second.dualWide) {
        if (diag) ++diag->dualWide;
      } else {
        if (diag) ++diag->singleWide;
      }
    }
  }
  ...
  for (auto *lane : lanes) {
    ...
    for (auto *pr : preds) {
      ...
      if (pit == table.end() || pit->second.wL.empty()) continue;
    ...
    for (auto *sc : succs) {
      ...
      if (sit == table.end() || sit->second.wL.empty()) continue;
    ...
    if (taper > total * 0.5) taper = total * 0.5;
    if (taper < 0.3) taper = std::min(total * 0.3, 0.5);
  ...
  double maxLaneW = params.max_lane_width;
  ...
  for (auto *lane : lanes) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (!ld.hasOpposite) hw = w_lane_;
      // max_lane_width 封顶（双开模式）
      if (ld.hasOpposite && ld.dualWide) {
        double cap = maxLaneW * 0.5;
        if (hw > cap) hw = cap;
      }
      if (hw < 0.2) hw = 0.2;
      halfW[i] = hw;
  ...
  for (auto *lane : lanes) {
    ...
    for (auto *pr : preds) {
      ...
      if (pit == bpts.end()) continue;
      BoundaryPts &bp = pit->second;
      if (bp.left.empty() || bp.right.empty()) continue;
  ...
  auto clearOldBoundary = [&](BoundSegment *bs) {
    if (!bs) return;
    if (m_boundlayer) m_boundlayer->DeleteMapFeature(bs);
    delete bs;
  };
  int processed = 0;
  for (auto *lane : lanes) {
    ...
    for (const auto &p : b.left) leftGeo->AppendVertex(p);
    for (const auto &p : b.right) rightGeo->AppendVertex(p);
  ...
  if (diag) diag->processed = processed;

  if (diag) diag->detail = os.str();
  return processed;
}
