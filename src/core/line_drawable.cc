...
void LineDrawable::add(float x, float y, float z, const V3f &color) {
  ...
  m_PointArray.push_back(
      PointColor(x, y, z, V4f(color[0], color[1], color[2], 1.0)));
}

void LineDrawable::add(const V3f &vec, const V3f &color) {
  m_boundBox.ExpandBy(vec[0], vec[1], vec[2]);
  m_PointArray.push_back(
      PointColor(vec, V4f(color[0], color[1], color[2], 1.0)));
}

void LineDrawable::addline(int start, int count) {
  m_lines.push_back(std::pair<int, int>(start, count));
}
...
void LineDrawable::DrawImplementation(RenderInfo &renderInfo) {
  ...
  if (m_PointArray.size() > 1) {
    ...
    glVertexAttribPointer(posLoc, 3, GL_FLOAT, false, sizeof(PointColor),
                          vertex);
    glVertexAttribPointer(clorLoc, 4, GL_FLOAT, false, sizeof(PointColor),
                          vertex + sizeof(V3f));
