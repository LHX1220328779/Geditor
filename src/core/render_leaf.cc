...
void RenderLeaf::SetModelViewMatrix(const Matrix4x4f *modelview) {
  m_modelview = *modelview;
}

void RenderLeaf::SetProjectionMatrix(const Matrix4x4f *projection) {
  m_projection = *projection;
}

void RenderLeaf::SetRenderTechnique(RenderTechnique *technique) {
  m_technique = technique;
}

void RenderLeaf::Render(RenderInfo &rendinfo, RenderLeaf *previous) {
  State *pState = rendinfo.GetState();
  if (m_technique != NULL && m_drawable->IsValid()) {
    ...
    for (int i = 0; i < nPassCount; i++) {
      ...
      m_drawable->SetParameter(rendinfo);
