...
class Database
{
    friend class Statement; // Give Statement constructor access to the sqLite_ptr_ Connection Handle
...
public:
    ...
    const char* getFilename() const  // nothrow
    {
        return filenames_for_database_;
    }
...
private:
    ...
    inline void check(const int aRet) const
    {
        if (SQLite::OK != aRet)
        {
            //throw SQLite::Exception(sqLite_ptr_, aRet);
        }
    }

private:
    sqlite3*     sqLite_ptr_;      ///< Pointer to SQLite Database Connection Handle
    char         filenames_for_database_[256];  ///< UTF-8 filename used to open the database
};
