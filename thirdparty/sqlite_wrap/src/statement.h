...
private:
    ...
    private:
        sqlite3*        sqLite_ptr_;    //!< Pointer to SQLite Database Connection Handle
        sqlite3_stmt*   sqlite_stmt_pointer_;      //!< Pointer to SQLite Statement Object
        unsigned int*   ref_count_pointer_;  //!< Pointer to the heap allocated reference counter of the sqlite3_stmt
...
private:
    ...
    inline void check(const int aRet) const
    {
        if (SQLite::OK != aRet)
        {
            //throw SQLite::Exception(stmt_ptr_, aRet);
        }
    }
    ...
    {
        if ((aIndex < 0) || (aIndex >= column_count_))
        {
...
private:
    std::string             query_;         //!< UTF-8 SQL Query
    Ptr                     stmt_ptr_;       //!< Shared Pointer to the prepared SQLite Statement Object
    int                     column_count_;   //!< Number of columns in the result of the prepared statement
    mutable TColumnNames    column_names_;   //!< Map of columns index by name (mutable so getColumnIndex can be const)
    bool                    if_row_fetched_;           //!< true when a row has been fetched with executeStep()
    bool                    last_executestep_done_;         //!< true when the last executeStep() had no more row to fetch
};
