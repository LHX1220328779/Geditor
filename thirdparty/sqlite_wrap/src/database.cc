...
Database::Database(const char* apFilename)
	:sqLite_ptr_(NULL)
{
	strcpy(filenames_for_database_, apFilename);
}
...
{
	const int ret = sqlite3_open_v2(filenames_for_database_, &sqLite_ptr_, aFlags, apVfs);
    if (SQLITE_OK != ret)
    {    		
		return false;   
	}
...
{
    const int ret = sqlite3_open(filenames_for_database_, &sqLite_ptr_);
    if (SQLITE_OK != ret)
    {
       return false;   
    }
...
void Database::close()
{
	...
	if(sqLite_ptr_ != NULL)
	{
		int ret = sqlite3_close(sqLite_ptr_);
		if(SQLITE_OK != ret)
		{
            std::cout<<"database is locked."<<std::endl;
		} 
		sqLite_ptr_ = NULL;
	}
}
...
{
    const int ret = sqlite3_busy_timeout(sqLite_ptr_, aBusyTimeoutMs);
    check(ret);
}
...
{
    const int ret = sqlite3_exec(sqLite_ptr_, apQueries, NULL, NULL, NULL);
    check(ret);

    // Return the number of rows modified by those SQL statements (INSERT, UPDATE or DELETE only)
    return sqlite3_changes(sqLite_ptr_);
}
...
{
    return sqlite3_last_insert_rowid(sqLite_ptr_);
}
                              ...
                              void        (*apDestroy)(void *))
{
    ...
    const int ret = sqlite3_create_function_v2(sqLite_ptr_, apFuncName, aNbArg, TextRep,
...
void Database::loadExtension(const char* apExtensionName, const char *apEntryPointName)
{
    ...
    int ret = sqlite3_db_config(sqLite_ptr_, SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION, 1, NULL);
#else
    int ret = sqlite3_enable_load_extension(sqLite_ptr_, 1);
#endif
    check(ret);

    ret = sqlite3_load_extension(sqLite_ptr_, apExtensionName, apEntryPointName, 0);
    check(ret);
#endif
}
...
void Database::key(const std::string& aKey) const
{
    ...
    if (pass_len > 0) {
        const int ret = sqlite3_key(sqLite_ptr_, aKey.c_str(), pass_len);
        check(ret);
    }
...
void Database::rekey(const std::string& aNewKey) const
{
    ...
    if (pass_len > 0) {
        const int ret = sqlite3_rekey(sqLite_ptr_, aNewKey.c_str(), pass_len);
        check(ret);
    } else {
        const int ret = sqlite3_rekey(sqLite_ptr_, nullptr, 0);
        check(ret);
    }
#else // SQLITE_HAS_CODEC
